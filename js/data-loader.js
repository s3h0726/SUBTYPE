...
export function normalizeStation(raw={},index=0,lineCode='ST'){
  ...
  return{...source,id:internalId,internalId,officialCode,stationCode:officialCode,hasOfficialStationCode:source.hasOfficialStationCode??!!officialCode,stationCodeSource:source.stationCodeSource||text(source.codeSource)||(officialCode?(explicitCode?'source-explicit':'bundled-verified-array'):'none'),ja,kana:text(source.kana,source.hiragana,fallbackName),romaji:text(source.romaji,source.en,fallbackName),ko:text(curated?.ko,source.nameKo,source.ko,fallbackName),koAliases:[...new Set(aliases)],koVerified:!!curated||!!source.koVerified,koSource:curated?'Tokyu official station list · pronunciation reviewed':source.koSource||'',latitude:Number.isFinite(latitude)?latitude:null,longitude:Number.isFinite(longitude)?longitude:null}
}
export function normalizeLine(raw={},defaults={}){
  ...
  const lineJa=cleanDisplayLineName(text(lineRaw.ja,raw.nameJa,raw.japanese,raw.name,lineRaw.en,'新路線')),operatorJa=text(operatorRaw.ja,raw.operatorNameJa,raw.operatorName,operatorRaw.en,'Railway'),operatorKo=operatorJa==='Osaka Metro'?'오사카 메트로':text(operatorRaw.ko,raw.operatorNameKo,raw.operatorName,operatorRaw.en,'철도');
  return{...raw,id,category,operatorId:text(raw.operatorId,operatorRaw.id)||category,operator:{ja:operatorJa,en:text(operatorRaw.en,raw.operatorNameEn,raw.operatorName,operatorRaw.ja,'Railway'),ko:operatorKo},line:{...lineRaw,ja:lineJa,en:text(lineRaw.en,raw.nameEn,raw.english,raw.name,lineRaw.ja,'New Line'),ko:LINE_KO_OVERRIDES[lineJa]||text(lineRaw.ko,raw.nameKo,raw.korean,raw.name,lineRaw.en,'새 노선')},code:resolvedCode,lineColor:lineTheme.color,loop:!!raw.loop,lineTheme,services,stations}
}
