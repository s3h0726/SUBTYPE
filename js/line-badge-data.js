...
(()=>{
  ...
  const makeAsset=(file,version='20260918')=>({asset:`${file}?v=${version}`,file,version,exists:true,verified:true,source:'',officialExists:true});
  ...
  const tokyuOperator=makeAsset('./data/operators/tokyu/logo.svg','20260917');
  const tokyuLines={
    'line-26001':commonsAsset('Tokyu TY line symbol.svg'),
    'line-26002':commonsAsset('Tokyu MG line symbol.svg'),
    'line-26003':commonsAsset('Tokyu DT line symbol.svg'),
    'line-26004':commonsAsset('Tokyu OM line symbol.svg'),
    'line-26005':commonsAsset('Tokyu IK line symbol.svg'),
    'line-26006':commonsAsset('Tokyu TM line symbol.svg'),
    'line-26007':commonsAsset('Tokyu SG line symbol.svg'),
    'line-26008':commonsAsset('Tokyu KD line symbol.svg'),
    'line-26009':commonsAsset('Tokyu SH line symbol.svg')
  };
  data.assets.operators.tokyu=tokyuOperator;
  Object.assign(data.assets.lines,tokyuLines);
  ...
  const tobuOperator=makeAsset('./data/operators/tobu/logo.svg','20260918');
  const tobuTJ=commonsAsset('Tobu Tojo Line (TJ) symbol.svg');
  const tobuTS=commonsAsset('Tobu Skytree Line (TS) symbol.svg');
  const tobuTI=commonsAsset('Tobu Isesaki Line (TI) symbol.svg');
  const tobuTN=commonsAsset('Tobu Nikko Line (TN) symbol.svg');
  const tobuTD=commonsAsset('Tobu Noda Line (TD) symbol.svg');
  const tobuLines={
    'line-21001':tobuTJ,
    'line-21002':tobuTS,
    'line-21003':tobuTN,
  ...
  data.assets.operators.tobu=tobuOperator;
  Object.assign(data.assets.lines,tobuLines);

  const seibuOperator=makeAsset('./data/operators/seibu/logo.svg','20260918');
  const seibuSI=commonsAsset('SeibuIkebukuro.svg');
  const seibuSS=commonsAsset('SeibuShinjuku.svg');
  const seibuSY=commonsAsset('SeibuYamaguchi.svg');
  const seibuSK=commonsAsset('SeibuKokubunji.svg');
  const seibuST=commonsAsset('SeibuTamako.svg');
  const seibuSW=commonsAsset('SeibuTamagawa.svg');
  const seibuLines={
    'line-22001':seibuSI,
    'line-22002':seibuSI,
    'line-22003':seibuSI,
  ...
  const odakyuOperator=makeAsset('./data/operators/odakyu/logo.svg','20260918');
  const odakyuLines={
    'line-25001':makeAsset('./assets/route-symbols/odakyu-oh.svg','20260918'),
    'line-25002':makeAsset('./assets/route-symbols/odakyu-oe.svg','20260918'),
    'line-25003':makeAsset('./assets/route-symbols/odakyu-ot.svg','20260918')
  };
  data.assets.operators.odakyu=odakyuOperator;
  ...
  for(const route of data.routes||[]){
    ...
    if(route.operatorId==='tokyu') route.operatorAsset=tokyuOperator;
    if(tokyuLines[route.id]){
      const asset=tokyuLines[route.id];
      route.symbolAsset=asset;
      route.symbolMeta={...(route.symbolMeta||{}),asset:asset.asset,officialSymbolExists:true,verified:false,identificationSource:'wikimedia-commons',assetSource:'wikimedia-commons',assetSourceUrl:asset.source};
      route.officialSymbolExists=true;
    }
    ...
    if(route.operatorId==='tobu') route.operatorAsset=tobuOperator;
    if(tobuLines[route.id]){
      const asset=tobuLines[route.id];
      route.symbolAsset=asset;
      route.symbolMeta={...(route.symbolMeta||{}),asset:asset.asset,officialSymbolExists:true,verified:false,identificationSource:'wikimedia-commons',assetSource:'wikimedia-commons',assetSourceUrl:asset.source};
      route.officialSymbolExists=true;
    }
    if(route.operatorId==='seibu') route.operatorAsset=seibuOperator;
    ...
    if(route.operatorId==='odakyu') route.operatorAsset=odakyuOperator;
    if(odakyuLines[route.id]){
      const asset=odakyuLines[route.id];
      route.symbolAsset=asset;
