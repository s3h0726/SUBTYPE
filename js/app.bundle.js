...
function servicePatternsForRoute(routeId){return(serviceCatalog().servicePatterns||[]).filter(item=>item.baseRouteId===routeId)}
...
const COUNTRY_UI={jp:{mark:'JAPAN RAIL',status:'日本全国 · JAPAN',subtitle:'일본 철도 타이핑',title:'일본 철도를<br><span>타이핑하다.</span>',intro:'전국의 실제 역 순서와 선형을 따라 달리는 철도 타이핑 게임.<br>현재역에서 다음역으로, 한국어 역명을 정확히 입력하세요.',network:'JAPAN RAILWAY NETWORK / 日本'},kr:{mark:'KOREA TRANSIT',status:'대한민국 · KOREA',subtitle:'대한민국 대중교통 타이핑',title:'대한민국 교통을<br><span>타이핑하다.</span>',intro:'철도·지하철·버스·수상교통의 실제 정차 순서를 따라가는 타이핑 게임.<br>한국어 역·정류장·선착장 이름을 정확히 입력하세요.',network:'KOREA PUBLIC TRANSPORT / 대한민국'}};
...
function renderTransportFilters(){const groupTabs=$('#transport-mode-tabs'),regionTabs=$('#region-tabs'),categoryTabs=$('#category-tabs');if(countryId==='jp'){groupTabs.hidden=true;regionTabs.hidden=true;categoryTabs.hidden=false;renderTabs(categoryTabs,JP_CATEGORIES,'category',category);return}groupTabs.hidden=false;renderTabs(groupTabs,KR_GROUPS,'transport-group',transportGroup);regionTabs.hidden=transportGroup!=='bus';if(transportGroup==='bus')renderTabs(regionTabs,KR_REGIONS,'region',regionFilter);const items=transportGroup==='bus'?busCategories():KR_CATEGORIES[transportGroup]||[];categoryTabs.hidden=!items.length;if(items.length)renderTabs(categoryTabs,items,'category',category)}
...
function allRoutes(){return[...builtin,...storage.routes().filter(route=>isCountryEnabled(route.countryId||'jp')).map(r=>railDataRepository.resolveRoute(normalizeLine(r,{category:'custom'})))].filter(r=>r.lazy||(Array.isArray(r.stations)&&r.stations.length>=2))}
...
function refreshRoutes(){routes=allRoutes();editor?.setRailNetwork?.(builtin);renderTransportFilters();renderOperatorFilters();renderRoutes();refreshFeatureCounts()}
