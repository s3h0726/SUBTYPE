/* AUTO-GENERATED by tools/build-static.js. DO NOT EDIT. */
(()=>{'use strict';(async()=>{
const $=(s,p=document)=>p.querySelector(s);
const $$=(s,p=document)=>[...p.querySelectorAll(s)];
const normalize=(v='')=>String(v).normalize('NFC').trim().replace(/\s+/g,' ').toLocaleLowerCase();
const uid=(prefix='id')=>`${prefix}-${globalThis.crypto?.randomUUID?.()||`${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`}`;
const clamp=(n,min,max)=>Math.max(min,Math.min(max,n));
const escapeHtml=(v='')=>String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
const formatTime=ms=>{ms=Math.max(0,Number(ms)||0);const m=Math.floor(ms/60000),s=Math.floor(ms%60000/1000),cs=Math.floor(ms%1000/10);return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${String(cs).padStart(2,'0')}`};
...
function resolveProjectAsset(relativePath){
...
const railDataRepository=new Repository();
...
function operatorLogoMarkup(routeOrOperatorId,className='operator-logo'){
...
const normalizeTypingValue=value=>String(value??'').normalize('NFC').replace(/\s+/gu,'');
class TypingController{
  constructor(input,{onChange,onSubmit}){this.input=input;this.onChange=onChange;this.onSubmit=onSubmit;this.composing=false;this.visualValue='';this.committedValue='';this.submitted=false;this.handlers={compositionstart:()=>{this.composing=true},compositionend:()=>{this.composing=false;this.evaluate()},beforeinput:event=>{if(event.inputType==='insertText'&&/^\s+$/u.test(event.data||''))event.preventDefault()},input:event=>{this.evaluate(this.input.value,false,this.composing||event.isComposing===true)},keydown:event=>{if(event.code==='Space'||event.key===' '){event.preventDefault();return}if(event.key==='Enter'&&!this.composing&&!event.isComposing)this.evaluate(this.input.value,true)}};Object.entries(this.handlers).forEach(([event,handler])=>input.addEventListener(event,handler))}
  setTarget(values){this.targets=[...new Set(values.filter(Boolean).map(normalizeTypingValue))];this.input.value='';this.visualValue='';this.committedValue='';this.submitted=false;this.input.closest('.typing-box')?.classList.remove('wrong','correct')}
  evaluate(rawValue=this.input.value,force=false,isComposing=this.composing){const raw=String(rawValue??'').normalize('NFC'),value=normalizeTypingValue(raw),target=this.targets?.[0]||'';if(raw!==value&&this.input.value===raw)this.input.value=value;this.visualValue=value;if(!isComposing)this.committedValue=value;const exact=(this.targets||[]).includes(value),prefix=(this.targets||[]).some(candidate=>candidate.startsWith(value)),state=!value?'empty':isComposing?'composing':exact?'correct':prefix?'typing':'wrong';this.onChange?.({value,visualValue:value,committedValue:this.committedValue,isComposing,state,target});if(isComposing||this.submitted)return;if(exact){this.submitted=true;this.input.closest('.typing-box')?.classList.add('correct');this.onSubmit?.(value)}else if(force||value&&!prefix)this.input.closest('.typing-box')?.classList.add('wrong');else this.input.closest('.typing-box')?.classList.remove('wrong')}
...
function isAutoFilledSymbol(character){return!PARENTHESES.has(character)&&!/[\p{L}\p{N}]/u.test(character)}
function buildTypingModel(value=''){
...
const keyByData={'jr-east':'jrEast','jr-central':'jrCentral','jr-west':'jrWest','jr-hokkaido':'jrHokkaido','jr-shikoku':'jrShikoku','jr-kyushu':'jrKyushu','tokyo-metro':'tokyoMetro','toei':'toei','osaka-metro':'osakaMetro','tokyu':'tokyu','odakyu':'odakyu','keio':'keio','seibu':'seibu','tobu':'tobu','keikyu':'keikyu','keisei':'keisei','sotetsu':'sotetsu','hankyu':'hankyu','hanshin':'hanshin','kintetsu':'kintetsu','keihan':'keihan','nankai':'nankai','meitetsu':'meitetsu','yurikamome':'yurikamome'};
function resolveStationSignTemplate(route){if(route.category==='shinkansen')return{key:'shinkansen',style:`shinkansen-${route.shinkansenStyle||'east'}`,family:'shinkansen',referenceSource:'JR operator station guides'};const explicit=keyByData[route.stationSignTemplate]||keyByData[route.operatorId]||route.stationSignTemplate;if(explicit&&templates[explicit])return{key:explicit,...templates[explicit]};const operator=route.operator?.ja||'',matched=rules.find(([pattern])=>pattern.test(operator));if(matched)return{key:matched[1],...templates[matched[1]]};if(route.category==='tram')return{key:'tram',...templates.tram};if(/モノレール|新交通/.test(`${operator}${route.line?.ja||''}`))return{key:'monorail',...templates.monorail};return{key:'local',...templates.local}}
const stationSignTemplates=templates;
...
function resolveTransportStopTemplate(route){
...
const transportStopTemplates=koreanTemplates;
...
const distanceKm=(a,b)=>{if(!validPoint(a)||!validPoint(b))return Infinity;const dLat=rad(b[0]-a[0]),dLon=rad(b[1]-a[1]),value=Math.sin(dLat/2)**2+Math.cos(rad(a[0]))*Math.cos(rad(b[0]))*Math.sin(dLon/2)**2;return 6371*2*Math.atan2(Math.sqrt(value),Math.sqrt(1-value))};
const cloneGeometry=geometry=>geometry.map(point=>[Number(point[0]),Number(point[1])]);

function canonicalStations(route){
...
function buildCanonicalSegmentMap(route,{endpointToleranceKm=1.2}={}){
...
function getDirectedSegment(segmentMap,fromId,toId){
...
function trainTypeFor(id){return serviceCatalog().trainTypes?.find(item=>item.id===id)||null}
function branchFor(id){return serviceCatalog().branches?.find(item=>item.id===id)||null}
function servicePatternFor(id){return serviceCatalog().servicePatterns?.find(item=>item.id===id)||null}
function servicePatternsForRoute(routeId){return(serviceCatalog().servicePatterns||[]).filter(item=>item.baseRouteId===routeId&&(!item.status||item.status==='active'))}
...
function serviceJourneysForRoute(routeId){
...
function journeyOriginsForRoute(routeId){return[...new Map(serviceJourneysForRoute(routeId).map(item=>[item.originStationId,{stationId:item.originStationId,names:item.originNames}])).values()]}
function journeyDestinationsForRoute(routeId,originStationId){return[...new Map(serviceJourneysForRoute(routeId).filter(item=>item.originStationId===originStationId).map(item=>[item.destinationStationId,{stationId:item.destinationStationId,names:item.destinationNames}])).values()]}
function journeyTrainTypesForRoute(routeId,originStationId,destinationStationId){return[...new Map(serviceJourneysForRoute(routeId).filter(item=>item.originStationId===originStationId&&item.destinationStationId===destinationStationId).map(item=>[item.trainTypeId,trainTypeFor(item.trainTypeId)])).values()].filter(Boolean)}
...
function buildThroughServiceRoute(spec,getRoute,directionId='forward'){
...
function resolveServiceJourney({baseRoute,journeyId,originStationId,destinationStationId,trainTypeId,getRoute}){
...
function setTrainTypingProgress(progress,motion=true){const rail=gameRail;if(!rail?.train||!rail.points.length)return;const cached=typingSegment(rail,rail.index,rail.nextIndex),{segment,lengths,total}=cached;if(segment.length<2)return;const safe=Math.max(0,Math.min(1,Number(progress)||0)),targetDistance=total*safe;let i=1;while(i<lengths.length&&lengths[i]<targetDistance)i++;const a=segment[Math.max(0,i-1)],b=segment[Math.min(i,segment.length-1)],span=Math.max(.0000001,lengths[i]-lengths[i-1]),part=(targetDistance-lengths[i-1])/span,target=[a[0]+(b[0]-a[0])*part,a[1]+(b[1]-a[1])*part],current=rail.train.getLatLng();animateTrain(rail,[current.lat,current.lng],target,motion);rail.typingProgress=safe;updateDebug()}
...
const transportStopNoun=route=>route?.mode==='river_bus'?'선착장':['bus','village_bus','express_bus'].includes(route?.mode)?'정류장':'역';
class Game{
  ...
  start(route,options={}){
    this.stop(false);route=railDataRepository.resolveRoute(route);if(!route||!Array.isArray(route.stations)||route.stations.length<2){console.error('Cannot start invalid line:',route?.id||route);toast('플레이하려면 역이 2개 이상인 노선이 필요합니다.');return false}
    this.route=route;this.options={mode:'complete',difficulty:'normal',display:'ja',input:'ko',direction:'forward',stationAdvance:'auto',mapMode:'geographic',stationLabel:'ja',mapLabels:'normal',motion:true,...options};this.service=route.services?.find(service=>service.id===this.options.service)||route.services?.find(service=>service.id==='local')||route.services?.[0]||{id:'local',nameJa:'各駅停車',nameKo:'각역정차',nameEn:'Local',stops:route.stations.map(station=>station.id)};this.servicePattern=this.options.servicePattern||null;this.serviceJourney=this.options.serviceJourney||route.serviceJourney||null;this.trainType=this.options.trainType||null;this.trainTypeContexts=this.options.trainTypeContexts||route.trainTypeContexts||[];this.destinationStationId=this.options.destinationStationId||this.service.destinationStationId||null;
    ...
    const sign=$('#station-sign'),template=resolveTransportStopTemplate(route);sign.dataset.signStyle=template.style;sign.dataset.templateFamily=template.family;sign.dataset.templateSource=template.referenceSource;const stopNoun=transportStopNoun(route),serviceLabel=this.serviceLabel();$('#typing-stop-label').textContent=`한국어 ${stopNoun} 이름 입력`;$('#typing-input').placeholder=`한국어 ${stopNoun} 이름을 입력하세요`;$('#following-stop-label').textContent=`그다음 ${stopNoun}`;$('#target-stop-label').textContent=`목표 ${stopNoun}`;this.applyActiveBrand(this.sequence[0]);$('#station-service-footer').textContent=`${serviceLabel} 운행`;$('#game-service-name').textContent=`${serviceLabel}${this.service.nameJa?` · ${this.service.nameJa}`:''}`;const origin=this.sequence[0],destination=this.sequence.at(-1);$('#service-origin-ja').textContent=origin.ja;$('#service-origin-ko').textContent=origin.ko;$('#service-destination-ja').textContent=destination.ja;$('#service-destination-ko').textContent=destination.ko;$('#service-stop-count').textContent=`${this.sequence.length}개 ${transportStopNoun(route)}`;$('#service-route-summary').textContent=`${route.line.ko} · ${serviceLabel}`;$('#service-direction-summary').textContent=`${origin.ko} → ${destination.ko}`;this.showDeparture();this.renderStation();this.updateHud();this.timer=setInterval(()=>this.tick(),50);this.typing.focus();return true
  }
  ...
  onInput({state,value,isComposing=false}){if(this.paused||this.phase!=='TYPING')return;const started=performance.now();this.renderKoreanProgress(value,state);const target=Array.from(this.answerValues(this.targetStation())[0]||''),typed=Array.from(value||'');let prefix=0;while(prefix<target.length&&typed[prefix]===target[prefix])prefix++;setTrainTypingProgress(this.boarding?0:target.length?prefix/target.length:0,this.options.motion&&!this.options.reducedMotion);const before=this.stats.mistakeUnits;if(!isComposing)this.stats.observe({value,state});this.errors=this.stats.mistakeUnits;if(this.errors>before){this.combo=0;$('#error-count').textContent=this.errors}$('#input-status').textContent=state==='wrong'?'CHECK':(state==='typing'||state==='composing')?`TYPING ${Math.round(prefix/Math.max(1,target.length)*100)}%`:'TYPE';$('#input-status').style.color=state==='wrong'?'#d93f3f':'';document.body.dataset.typingUpdateMs=(performance.now()-started).toFixed(2);this.updateHud()}
  correct(){
    if(this.paused||this.finished||this.phase!=='TYPING')return;const station=this.targetStation();if(!station)return;this.phase='ARRIVING';this.stats.complete(this.answerValues(station)[0]||'');this.typedStationIds.push(station.id);this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);const manual=this.options.stationAdvance==='space';setTrainTypingProgress(this.boarding?0:1,this.options.motion&&!this.options.reducedMotion);$('#typing-input').readOnly=true;$('#typing-input').closest('.typing-box').classList.add('waiting');$('#input-status').textContent=manual?'SPACE NEXT':this.boarding?'DEPARTING':'ARRIVING';$('#next-station-button').hidden=!manual;this.phase=manual?'CORRECT_WAITING':'ARRIVING';this.playTone();this.updateHud();this.syncDebug();if(!manual)queueMicrotask(()=>this.advance())
  }
  advance(){
    if(this.finished||this.paused||!['CORRECT_WAITING','ARRIVING'].includes(this.phase))return;this.phase='MOVING';$('#next-station-button').hidden=true;const arrived=this.targetStation();if(!arrived)return this.finish();if(this.boarding){this.boarding=false;const current=this.currentStation();renderGameMap(this.mapRoute,this.routeIndex(current),this.mapOptions());this.updateHud();this.syncDebug();this.renderStation();return}this.advanceToNextStation(arrived);const current=this.currentStation();renderGameMap(this.mapRoute,this.routeIndex(current),this.mapOptions());this.updateHud();this.syncDebug();if((this.options.mode==='random'&&this.elapsed()>=60000)||!this.targetStation())return this.finish();this.renderStation()
  }
...
function normalizeStation(raw={},index=0,lineCode='ST'){
...
function normalizeLine(raw={},defaults={}){
...
async function loadTransportData(countryId='jp'){
...
class RouteEditor{
 ...
 renderSearch(value){const target=$('#station-search-results');if(!target)return;const q=normalize(value);if(!q){target.innerHTML='<p>한국어·日本語·かな·Romaji 역 이름을 입력하세요.</p>';return}const matches=this.master.map(station=>{const names=[station.ja,station.kana,station.romaji,station.ko,...(station.koAliases||[])].map(normalize),all=normalize([names.join(' '),...(station.operators||[]),...(station.lines||[]).flatMap(line=>[line.lineJa,line.lineKo,line.operatorJa])].join(' ')),rank=names.includes(q)?0:names.some(name=>name.startsWith(q))?1:names.some(name=>name.includes(q))?2:all.includes(q)?3:99;return{station,rank}}).filter(item=>item.rank<99).sort((a,b)=>a.rank-b.rank||a.station.ko.localeCompare(b.station.ko,'ko')).slice(0,12).map(item=>item.station);target.innerHTML=matches.length?matches.map(station=>`<article class="station-search-result"><div><b>${escapeHtml(station.ko||'MISSING_KOREAN_NAME')}</b><span>${escapeHtml(station.ja)}</span><small>${escapeHtml(station.romaji)} · ${escapeHtml((station.operators||[]).join(' · '))}</small><small>${escapeHtml((station.lines||[]).slice(0,5).map(line=>line.lineKo||line.lineJa).join(' · '))}${station.lines?.length>5?' 외':''}</small></div><button type="button" data-add-master="${escapeHtml(station.id)}">＋ 추가</button></article>`).join(''):'<p>일치하는 실제 역이 없습니다.</p>'}
