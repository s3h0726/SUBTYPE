...
class TypingController{
  constructor(input,{onChange,onSubmit}){this.input=input;this.onChange=onChange;this.onSubmit=onSubmit;this.composing=false;this.visualValue='';this.committedValue='';this.submitted=false;this.handlers={compositionstart:()=>{this.composing=true},compositionend:()=>{this.composing=false;this.evaluate()},input:event=>{this.evaluate(this.input.value,false,this.composing||event.isComposing===true)},keydown:event=>{if(event.key==='Enter'&&!this.composing&&!event.isComposing)this.evaluate(this.input.value,true)}};Object.entries(this.handlers).forEach(([event,handler])=>input.addEventListener(event,handler))}
  setTarget(values){this.targets=[...new Set(values.filter(Boolean).map(value=>String(value).normalize('NFC')))];this.input.value='';this.visualValue='';this.committedValue='';this.submitted=false;this.input.closest('.typing-box')?.classList.remove('wrong','correct')}
  evaluate(rawValue=this.input.value,force=false,isComposing=this.composing){const value=String(rawValue).normalize('NFC'),target=this.targets?.[0]||'';this.visualValue=value;if(!isComposing)this.committedValue=value;const exact=(this.targets||[]).includes(value),prefix=(this.targets||[]).some(candidate=>candidate.startsWith(value)),state=!value?'empty':isComposing?'composing':exact?'correct':prefix?'typing':'wrong';this.onChange?.({value,visualValue:value,committedValue:this.committedValue,isComposing,state,target});if(isComposing||this.submitted)return;if(exact){this.submitted=true;this.input.closest('.typing-box')?.classList.add('correct');this.onSubmit?.(value)}else if(force||value&&!prefix)this.input.closest('.typing-box')?.classList.add('wrong');else this.input.closest('.typing-box')?.classList.remove('wrong')}
...
function buildThroughServiceRoute(spec,getRoute){
...
const COUNTRY_UI={jp:{mark:'JAPAN RAIL',status:'東京首都圏 · TOKYO AREA',subtitle:'도쿄권 철도 타이핑',title:'도쿄권 철도를<br><span>타이핑하다.</span>',intro:'도쿄·가나가와·사이타마·치바의 실제 역 순서와 선형을 따라 달리는 철도 타이핑 게임.<br>현재역에서 다음역으로, 한국어 역명을 정확히 입력하세요.',network:'TOKYO AREA RAILWAY NETWORK / 首都圏'},kr:{mark:'KOREA TRANSIT',status:'대한민국 · KOREA',subtitle:'대한민국 대중교통 타이핑',title:'대한민국 교통을<br><span>타이핑하다.</span>',intro:'철도·지하철·버스·수상교통의 실제 정차 순서를 따라가는 타이핑 게임.<br>한국어 역·정류장·선착장 이름을 정확히 입력하세요.',network:'KOREA PUBLIC TRANSPORT / 대한민국'}};
...
function refreshCountryCopy(){const korea=countryId==='kr',play=document.querySelector('.launch-menu [data-nav="rail-map"] small'),footer=document.querySelector('.home-footer span:nth-child(2)'),featureButtons=$$('#feature-viewport [data-feature-route]');if(play)play.textContent=korea?'대한민국 교통에서 노선 선택':'도쿄권 철도에서 노선 선택';if(footer)footer.textContent=korea?'대한민국 대중교통 타이핑':'도쿄권 철도 타이핑';if(featureButtons[0])featureButtons[0].dataset.featureRoute=korea?'kr-seoul-line-2':'line-11302';if(featureButtons[1])featureButtons[1].dataset.featureRoute=korea?'kr-shinbundang':'line-28001'}
...
$('#setup-form').addEventListener('submit',e=>{e.preventDefault();const data=new FormData(e.target),settings=storage.settings(),stationLabel=['ja','ja-romaji'].includes(settings.stationLabel)?settings.stationLabel:'ja',direct=throughSpec(selected),patternId=String(data.get('service')||''),patterns=servicePatternsForRoute(selected.id);let resolved=null;try{resolved=resolveServiceSelection({baseRoute:selected,servicePatternId:patterns.some(item=>item.id===patternId)?patternId:null,legacyServiceId:patternId,directionId:$('#service-direction').value,getRoute:id=>builtin.find(route=>route.id===id)})}catch(error){console.error('Service pattern resolution failed:',error);return toast(`운행계통 오류: ${error.message}`)}lastRunRoute=data.get('directService')==='on'&&direct&&!patterns.length?buildThroughRoute(direct):resolved.route;if(!lastRunRoute)return toast('직결운행 데이터를 준비하지 못했습니다.');lastOptions={mode:data.get('mode'),inputMode:data.get('inputMode')||'shadowing',difficulty:data.get('difficulty'),service:resolved.service?.id||patternId,direction:resolved.direction,display:'ja',input:'ko',stationAdvance:data.get('stationAdvance')||settings.stationAdvance,mapMode:$('#game-map-mode').value,stationLabel,mapLabels:settings.mapLabels,motion:settings.motion,reducedMotion:settings.reducedMotion,sound:settings.sound,servicePattern:resolved.pattern,trainType:resolved.trainType,destinationStationId:resolved.destinationStationId,throughServiceId:resolved.pattern?.throughServiceId||lastRunRoute.throughService?.id||null};if(lastRunRoute.dataKind==='throughService'&&!resolved.pattern){lastOptions.service='local';lastOptions.direction='forward'}$('#map-view-toggle').textContent=lastOptions.mapMode==='geographic'?'SCHEMATIC':'GEOGRAPHIC';if(game.start(lastRunRoute,lastOptions)){go('game');game.focusInput()}});
