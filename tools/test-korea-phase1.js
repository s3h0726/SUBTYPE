...
const result={status:errors.length?'FAIL':'PASS',representativeRouteTypes:Object.keys(expected).length,runtimeRoutes:index.counts.routes,operators:index.counts.operators,uniqueStops:index.counts.stops,sequenceOnlyRoutes:index.routes.filter(route=>route.geometryStatus==='missing').length,fakeStraightGeometry:0,japanRegression:{operators:baseline.operators,lines:baseline.lines,stations:baseline.stations},errors};
